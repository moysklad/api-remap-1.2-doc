...
С 1 декабря 2023 домен `online.moyslad.ru` перестанет обслуживать запросы к `/api/remap/1.2`. Запросы нужно будет отправлять на домен `api.moyslad.ru`. 

До 1 декабря 2023 года обслуживать запросы будут оба вышеуказанных домена. С единственным отличием:

  + В запросах на https://online.moyslad.ru/api/remap/1.2 указание заголовка «Accept-Encoding»: «gzip» опционально
  + В запросах на https://api.moysklad.ru/api/remap/1.2 указание заголовка «Accept-Encoding»: «gzip» **обязательно**

Чтобы работа ваших приложений не прерывалась, рекомендуем начать подготовку к переезду на новый домен `api.moyslad.ru` _уже сейчас_. 

1. Настройте передачу заголовка `Accept-Encoding: gzip` в запросах и чтение gzip-ответов на текущем домене `online.moyslad.ru`.
2. Проверьте, как ваша интеграция использует ссылки (поля `href`), передаваемые в объектах `meta` и в вебхуках. Если вы отправляете запросы, используя информацию из получаемых ссылок, то обратите внимание на то, что:
   + при выполнении запросов на домен `online.moyslad.ru`, в `meta` возвращаются ссылки с доменом `online.moyslad.ru`. А при выполнении запросов на домен `api.moyslad.ru`, в `meta` возвращаются ссылки с доменом `api.moyslad.ru`.
   + webhook, созданный через запрос к домену `online.moyslad.ru` будет содеражть в теле ссылки (поля `href`) с доменом на `online.moyslad.ru`. А webhook, созданный через запрос к домену `api.moyslad.ru` будет содеражть в теле ссылки с доменом на `api.moyslad.ru`
3. Когда ваша интеграция будет готова передавать заголовок «Accept-Encoding»: «gzip» и читать gzip-ответы
   + замените адрес https://online.moyslad.ru/api/remap/1.2 на https://api.moysklad.ru/api/remap/1.2
