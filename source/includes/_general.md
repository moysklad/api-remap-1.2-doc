...
с 1 декабря 2023 использование адреса https://api.moysklad.ru/api/remap/1.2 и указание заголовка «Accept-Encoding»: «gzip» при запросах к JSON API 1.2 станет обязательным.

До 1 декабря 2023 года оба домена работают в обычном режиме с единственным отличием:

  + https://online.moyslad.ru/api/remap/1.2 → указание заголовка «Accept-Encoding»: «gzip» опционально
  + https://api.moysklad.ru/api/remap/1.2 → указание заголовка «Accept-Encoding»: «gzip» обязательно

Чтобы работа ваших приложений была бесшовной, подготовку к переезду можно начать уже сейчас.

1. Настройте передачу заголовка «Accept-Encoding»: «gzip» и чтение gzip-ответов. Некоторые популярные библиотеки поддерживают формат gzip по умолчанию. Например, guzzle (PHP), axios (node.js), requests (python). Если вы пользуетесь настройками по умолчанию одной из этих библиотек, то дополнительных действий для чтения gzip-ответов не требуется. Иначе: включите передачу заголовка и чтение gzip-ответов.
2. Проверьте, как ваша интеграция использует ссылки, передаваемые в meta и вебхуках. Если вы выполняете запросы, используя полученные ссылки, обратите внимание
   + при выполнении запросов на домен https://online.moyslad.ru/api/remap/1.2, в meta вернутся ссылки на https://online.moyslad.ru/api/remap/1.2 (и наоборот)
   + если webhook зарегистрирован с использованием домена https://online.moyslad.ru/api/remap/1.2, то вернутся ссылки на https://online.moyslad.ru/api/remap/1.2 (и наоборот)
3. Когда ваша интеграция будет готова передавать заголовок «Accept-Encoding»: «gzip» и читать gzip-ответы
   + замените адрес https://online.moyslad.ru/api/remap/1.2 → https://api.moysklad.ru/api/remap/1.2
   + перерегистрируйте вебхуки, используя новый домен
