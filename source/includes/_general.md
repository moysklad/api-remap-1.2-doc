...
2. Проверьте использование в вашей интеграции ссылок (полей `href`), передаваемых в объектах `meta` и в вебхуках. Если вы отправляете запросы, используя информацию из получаемых ссылок, то обратите внимание на то, что:
   ...
   + до 1 декабря вебхук, созданный через запрос к домену `online.moysklad.ru` будет содержать в теле ссылки (поля `href`) с доменом на `online.moysklad.ru`, а вебхук, созданный через запрос к домену `api.moysklad.ru` будет содержать в теле ссылки с доменом `api.moysklad.ru`. После 1 декабря все вебхуки будут содержать в теле ссылки с доменом `api.moysklad.ru`, независимо от домена регистрации. Сейчас, чтобы проверить корректность обработки ссылок с новым доменом в теле вебхука, вы можете зарегистрировать новый вебхук, используя новый домен. Чтобы определить принадлежность вебхука домену, вы можете на [эндпоинте получения вебхуков](documents/#suschnosti-vebhuki) использовать временно введенный параметр запроса `oldDomain`. `oldDomain = true` - отображает вебхуки старого домена, `oldDomain = false` - отображает вебхуки домена нового.
