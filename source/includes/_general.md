...
Для изменения времени жизни URL файла, нужно в запросе передать специальный заголовок `X-Lognex-Download-Expiration-Seconds` и в значении указать необходимое время в секундах. 
Заголовок принимает целое число от 1 до 86400 (24 часа). Рекомендуется не использовать верхнее граниченое значение без необходимости.
