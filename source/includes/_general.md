...
2. Проверьте, как ваша интеграция использует ссылки (поля `href`), передаваемые в объектах `meta` и в вебхуках. Если вы отправляете запросы, используя информацию из получаемых ссылок, то обратите внимание на то, что:
   ...
   + webhook, созданный через запрос к домену `online.moysklad.ru` будет содержать в теле ссылки (поля `href`) с доменом на `online.moysklad.ru`. А webhook, созданный через запрос к домену `api.moysklad.ru` будет содеражть в теле ссылки с доменом на `api.moysklad.ru`
