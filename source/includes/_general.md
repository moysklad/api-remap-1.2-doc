...
Для JSON API установлены следующие ограничения:
  ...
  + Обязательное использование [сжатия содержимого запросов](#mojsklad-json-api-obschie-swedeniq-szhatie-soderzhimogo-zaprosow)
...
### Сжатие содержимого запросов

Сервер апи использует сжатие содержимого запросов, поэтому при обращении к эндпоинтам апи через HTTP-заголовок запроса Accept-Encoding следует указать кодировку сжатия содержимого запросов.

###### Поддерживаемые кодировки сжатия:
+ gzip
+ compress
+ deflate
+ br
