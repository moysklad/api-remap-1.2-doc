...
С 1 декабря 2023 домен `online.moysklad.ru` перестанет обслуживать запросы к `/api/remap/1.2`. Запросы нужно будет отправлять на домен `api.moysklad.ru`. 

До 1 декабря 2023 года обслуживать запросы будут оба вышеуказанных домена. С единственным отличием:

  + В запросах на https://online.moysklad.ru/api/remap/1.2 указание заголовка «Accept-Encoding»: «gzip» опционально
  + В запросах на https://api.moysklad.ru/api/remap/1.2 указание заголовка «Accept-Encoding»: «gzip» **обязательно**
...
2. Проверьте, как ваша интеграция использует ссылки (поля `href`), передаваемые в объектах `meta` и в вебхуках. Если вы отправляете запросы, используя информацию из получаемых ссылок, то обратите внимание на то, что:
   + при выполнении запросов на домен `online.moysklad.ru`, в `meta` возвращаются ссылки с доменом `online.moysklad.ru`. А при выполнении запросов на домен `api.moysklad.ru`, в `meta` возвращаются ссылки с доменом `api.moysklad.ru`.
   + до 1 декабря webhook, созданный через запрос к домену `online.moysklad.ru` будет содержать в теле ссылки (поля `href`) с доменом на `online.moysklad.ru`, а webhook, созданный через запрос к домену `api.moysklad.ru` будет содеражть в теле ссылки с доменом `api.moysklad.ru`. После 1 декабря все webhook'и будут содержать в теле ссылки с доменом `api.moysklad.ru`, независимо от домена регистрации. Сейчас, чтобы проверить корректность обработки ссылок с новым доменом в теле webhook'а, вы можете зарегистрировать новый webhook, используя новый домен. Чтобы определить принадлежность webhook'а к домену, вы можете на [эндпоинте получения webhook'ов](documents/#suschnosti-vebhuki) использовать временно введенный параметр запроса `OldDomain`. `OldDomain = true` - отображает webhook'и старого домена, `OldDomain = false` - webhook'и домена нового.
