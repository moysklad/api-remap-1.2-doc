...
+ meta - [Метаданные](#metadannye), представляющие собой ссылку на сущность из соответствующего справочника
...
Пример указания значения для доп. поля типа файл есть в секции [создания товара](#izmenit-towar)
...
+ С миллисекундами: `ГГГГ-ММ-ДД ЧЧ:мм:сс.ммм` В этом формате возвращается время в ответах на запросы.

Параметры фильтрации для полей типа дата, например `moment`, можно указывать как в выше обозначенных форматах, так и без секунд: `ГГГГ-ММ-ДД ЧЧ:мм`.
...
Ссылки выводятся в формате [Метаданных](#metadannye). Для того, чтобы вместо ссылок получить связанные объекты,
...
контрагента будет выведен объект со всеми полями сущности "Контрагент", описанными [тут](#kontragent).
