...
### Оптимизация работы с JSON API в части запросов

Частой проблемой исчерпания лимитов по запросам является плохо настроенная или не продуманная интеграция.
...
Часто бывает, что за небольшой промежуток времени приходит много дублирующихся запросов. Это может быть связано:

* с наличием нескольких экземпляров одного и того же приложения, каждое из которых не знает о работе соседнего
* с плохо настроенными повторами запросов:
  * запрос повторяется не получив ответа от предыдущего запроса. К примеру на ответ отводится 5 секунд, а затем идет повторная отправка.
...
* отсутствие кэширования вложенных объектов при получении списка сущностей

#### Проблема избыточных запросов

Одной из основных проблем, которые приводят к повышенному количеству запросов - плохо продуманная интеграция. 
Там где можно справиться одним запросом, используется несколько. Основные примеры:
