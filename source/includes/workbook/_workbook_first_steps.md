...
### Настройки переиспользования соединений

Чтобы не попасть в бан по IP-адресу по признаку `SYN/ACK Flood Attack`, рекомендуется настроить переиспользование соединений (HTTP keep-alive/HTTP persistent connection).

Рекомендуется следующая конфигурация переиспользования соединений:

- Время жизни неактивного соединения до 33 секунд
- До 100 запросов на одно соединение.

Если запрос идет не напрямую на api.moysklad.ru, а через proxy-сервер, необходимо применить указанную выше конфигурацию на стороне proxy-сервера и расширить ее:

- До 33 количество одновременных неактивных соединений к api.moysklad.ru.
