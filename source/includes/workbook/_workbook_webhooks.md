...
Вебхук будет автоматически отключен, если попадает под одно из правил (могут дополняться и уточняться):

* более семи дней из последних десяти дней сервис, принимающий вебхуки, отвечал только кодами ошибок (либо был недоступен), **и** сервис ни разу не обработал успешно вебхук за последние десять дней.
* отправлено много вебхуков на сервис для тестирования вебхуков (например webhook.site)
