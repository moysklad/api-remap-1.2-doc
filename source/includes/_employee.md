...
буду отсутствовать, кроме `view` равное `all` для `currency`, `country` и `uom`. В случае отсутствия поля `permissions` или 
если оно пустое, то будут заданы значения пермиссий по умолчанию.

В случае, если у пользователя уже были заданы ранее индивидуальные настройки, то:

* при отсутствии поле `permissions`, индивидуальные пермисии останутся такими же как были ранее
