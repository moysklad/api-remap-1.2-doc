...
Если у пользователя ранее не было задано индивидуальных пермиссий, то при указании пермиссий в `permissions`, остальные пермисии 
будут отсутствовать, кроме `view` равное `all` для `currency`, `country` и `uom`. В случае отсутствия поля `permissions` или 
если значение поля пустое, то будут заданы значения пермиссий по умолчанию.

В случае, если у пользователя уже были заданы ранее индивидуальные настройки, то:

* при отсутствии поля `permissions`, индивидуальные пермисии останутся такими же как были ранее
* при пустом поле `permissions`, то будут заданы значения пермиссий по умолчанию
* в остальных случаях ранее заданные индивидуальные пермиссии будут изменены в соответствии с переданными в поле `permissions` значениями
