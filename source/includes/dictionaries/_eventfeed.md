...
Средствами JSON API можно запрашивать сведения по Ленте Событий для некоторых типов документов, в виде списка или отдельных Событий.

Лента Событий работает со следующими типами документов:
+ [Заказ покупателя](../documents/#dokumenty-zakaz-pokupatelq)

#### Работа с упоминаниями в Ленте Событий

События в Ленте могут содержать упоминания других сотрудников прямо в тексте самого События. 
...
Поддерживаемые типы в упоминаниях:
...
| Параметр   | Описание                                                                                                                             |
|------------|:-------------------------------------------------------------------------------------------------------------------------------------|
| **type**   | `string` (required) *Example: customerorder* тип документа, для которого запрашиваются Событий.                                      |
| **id**     | `string` (required) *Example: 7944ef04-f831-11e5-7a69-971500188b19* id документа c Событиями.                                        |
| **limit**  | `number` (optional) **Default: 100** *Example: 100* Максимальное количество сущностей для извлечения.`Допустимые значения 1 - 1000`. |
