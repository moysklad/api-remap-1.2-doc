...
На платных тарифих есть возможность получить постоянную ссылку для скачивание изображения. Для этого необходимо указать дополнительный параметр `fields=downloadPermanentHref`. Параметр влияет 
на вывод поля meta у Изображния, поэтому его можно применять вместе с параметром `expand=images`.

Получить изображение по ссылке можно без авторизации.
