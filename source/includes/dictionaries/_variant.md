...
| **barcodes**           | Array(Object)                                             | `=` `!=` `~` `~=` `=~`                            | Массив штрихкодов модификации. [Подробнее тут](../dictionaries/#suschnosti-modifikaciq-modifikacii-atributy-wlozhennyh-suschnostej-shtrihkody).  Для фильтрации по полю необходимо указывать его в единственном числе **barcode**.            |
...
| **minimumStock**       | Object                                                    |                             | Неснижаемый остаток. [Подробнее тут](../dictionaries/#suschnosti-modifikaciq-modifikacii-atributy-wlozhennyh-suschnostej-nesnizhaemyj-ostatok)<br>`+Выводится по запросу`                                                                   |
...
##### Неснижаемый остаток
Неснижаемый остаток — минимальное количество модификации, которое всегда должно быть на складе.

Типы Неснижаемого остатка:

+ ALL_WAREHOUSE_SUM (В сумме на всех складах)
+ ALL_WAREHOUSE_SAME (Одинаковый на всех складах)
...
[//]: # (TODO Строку ниже раскоментировать, и нужно добавить ссылку на раздел с фичей fields. Раздел добавится в тикете https://lognex.atlassian.net/browse/DND-2905)
[//]: # (Особенности: поле Неснижаемого остатка выводится только, если пользователь API сам пожелал увидеть его в выводе. )

###### Атрибуты Неснижаемого остатка с типом ALL_WAREHOUSE_SUM

| Название      | Тип     | Описание                                                                                         |
|---------------|:--------|:-------------------------------------------------------------------------------------------------|
| **type**      | String  | Разновидность Неснижаемого остатка<br>`+Обязательное при ответе` `+Только для чтения`            |
| **inherited** | Boolean | Признак, наследуется ли неснижаемый остаток с родительского товара<br>`+Обязательное при ответе` |
| **quantity**  | Double  | Количество Неснижаемого остатка в сумме на всех складах<br>`+Обязательное при ответе`            |
...
| **type**      | String  | Разновидность Неснижаемого остатка<br>`+Обязательное при ответе` `+Только для чтения`            |
| **inherited** | Boolean | Признак, наследуется ли неснижаемый остаток с родительского товара<br>`+Обязательное при ответе` |
| **quantity**  | Double  | Количество Неснижаемого остатка одинаковое для каждого склада<br>`+Обязательное при ответе`      |
...
| **type**          | String    | Разновидность Неснижаемого остатка<br>`+Обязательное при ответе` `+Только для чтения`            |
| **inherited**     | Boolean   | Признак, наследуется ли неснижаемый остаток с родительского товара<br>`+Обязательное при ответе` |
| **storebalances** | MetaArray | Количество Неснижаемого остатка заданное для каждого склада<br>`+Обязательное при ответе`        |

Структура объекта в коллекции **storebalances**:

| Название     | Тип                                                       | Описание                                                                                                          |
|--------------|:----------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------|
| **meta**     | [Meta](../#mojsklad-json-api-obschie-swedeniq-metadannye) | Метаданные Неснижаемого остатка<br>`+Обязательное при ответе` `+Только для чтения`                                |
| **store**    | [Meta](../#mojsklad-json-api-obschie-swedeniq-metadannye) | Метаданные склада для которого задан Неснижаемый остаток<br>`+Обязательное при ответе` `+Необходимо при создании` |
| **quantity** | Double                                                    | Количество Неснижаемого остатка для склада<br>`+Обязательное при ответе` `+Необходимо при создании`               |

Для создания или изменения Неснижаемого остатка для склада (складов) можно передать в теле запроса на [создание](../dictionaries/#suschnosti-modifikaciq-sozdat-modifikaciu)
или [обновление](../dictionaries/#suschnosti-modifikaciq-izmenit-modifikaciu) модификации Неснижаемые остатки с указанием склада и количества неснижаемого остатка для данного склада.
