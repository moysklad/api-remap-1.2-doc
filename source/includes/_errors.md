...
|1033|Ошибка фильтрации: фильтрация по характеристикам модификаций совместима только с группировкой по модификациями|В отчетах при передаче значения параметра groupBy отличного от `variant` невозможна фильтрация по характеристикам модификаций.
...
|3014|Ошибка валидации: поля '{имя поля}' и '{имя поля}' не могут одновременно иметь значение {имя параметра}|Нельзя, чтобы два параметра одновременно имели одинаковое значение 
...
|4001|Ошибка сохранения объекта: превышен допустимый лимит активных точек продаж по вашему тарифу|Число активных точек продаж превышает лими по тарифу.
