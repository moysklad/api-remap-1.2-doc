...
|1085|Используемый тип авторизации не содержит информацию о пользователе|Необходим иной способ авторизации, содержащий в себе информацию о пользователе. Например, [basic или токен](https://dev.moysklad.ru/doc/api/remap/1.2/#mojsklad-json-api-obschie-swedeniq-autentifikaciq)
...
|3014|Ошибка валидации штрихкода: неверный формат GTIN: <текст ошибки>|Ошибка валидации штрихкода типа GTIN. Тексты ошибок: <ul><li>GTIN должен содержать только цифры</li> <li>В штрихкоде GTIN должно быть 14, 13. 12 или 8 цифр</li> <li>Не сходится контрольная сумма
...
|3016|Ошибка валидации: Поле '{имя поля}' должно соответствовать формату '{ожидаемый формат поля}'|Ошибка валидации поля. Входящее поле должно соответствовать ожидаемому формату</li> </ul>
|3017|Ошибка валидации: Сумма накоплений должна быть больше предыдущей | Ошибка валидации поля levels. В накопительных скидках сумма каждого накопления должна быть больше предыдущих</li> </ul>
|3018|Ошибка валидации: Скидка должна быть больше предыдущей | Ошибка валидации поля levels. В накопительных скидках значение скидки должно быть больше предыдущих</li> </ul>
|3019|Ошибка валидации: Суммы должны различаться |  Ошибка валидации поля levels. В накопительных скидках значение скидки должно отличаться от предыдущих</li> </ul>
|3020|Ошибка валидации: Другая бонусная программа уже активна' |Ошибка при активации бонусной программы. Невозможно активировать бонусную программу, если другая уже активна</li> </ul>
|3021|Ошибка валидации: Если в скидке указано использовать специальную цену, то необходимо заполнить поле specialPrice. Иначе необходимо заполнить поле discount. '| Ошибка валидации спциальных цен. Данная скидка должна содержать поля specialPrice или discount, если указан соответствующий флаг</li> </ul>
|3022|Ошибка валидации: Превышен лимит по количеству скидок | Ошибка при активации скидки. В системе не может быть больше 10 активных скидок одновременно</li> </ul>                                                  
|3024|Ошибка валидации: Ошибка валидации: неверное значение '{значение поля}' поля '{имя поля}'. Допустимые значения: {список_значений}" |У поля указано не валидное значение</li> </ul>                                                  
|3025|Ошибка валидации: Ошибка валидации: не заполнено обязательное поле '{имя поля}' у объекта '{имя объекта}' | У объекта в json'е не указано обязательное поле</li> </ul>                                                  
...
|16106|Ошибка сохранения товара: неизвестный тип маркировки товара|Тип маркировки товара может быть только одним из следующих: NOT_TRACKED, TOBACCO, SHOES, LP_CLOTHES, LP_LINENS, PERFUMERY, ELECTRONICS, TIRES, OTP
...
|17102|Ошибка сохранения документа: неверный формат кода маркировки <значение КМ / КМ ТУ / КМ ПУ>|Код маркировки должен соответствовать формату:<ul><li>Обувная продукция: 01+GTIN+21+SERIAL. GTIN - идентификатор товара из 14 цифр, SERIAL - серийный номер из 13 символов. Пример: <b><font color='green'>01</font>04630037590265<font color='green'>21</font>K6UT)6\"pIG-<</b></li><li>Табачная продукция продукция, пачка: GTIN+SERIAL. GTIN - идентификатор товара из 14 цифр, SERIAL - серийный номер из 7 символов</li><li>Табачная продукция продукция, блок (упаковка): 01+GTIN+21+SERIAL. GTIN - идентификатор товара из 14 цифр, SERIAL - серийный номер из 7 символов. Пример: <b>01</b>04630034070029<b>21</b>tXXXu%d</li><li>Код маркировки транспортной упаковки продукции должен содержать 18 цифр - формат GS1</li></ul>
...
|43011|Ошибка обновления прав сотрудника: для пермиссии {название пермиссии} задано значение для {значение пермиссии}, но не указано значение для {название пермиссии}|Нельзя задавать значение пермисии, не указавать значения для родительской пермиссии.
|43012|Ошибка обновления прав сотрудника: для пермиссии {тип пермиссии} задано значение для {название пермиссии} {значение пермиссии}, но для {название пермиссии} указано {значение пермиссии}|Указаны несовместимые типы для дочерней и родительской пермиссии.
|43013|Ошибка обновления прав сотрудника: невозможно изменить системную роль 'Администратор аккаунта' у последнего администратора|Необходимо, чтобы на аккаунте всегда была хотя бы один сотрудник с ролью 'Администратор аккаунта' 
...
|43016|Ошибка сброса пароля сотрудника: у пользователя нет или указан не верный email|у пользователя нет или указан не верный email у сотрудника.
...
|43019|Ошибка деактивации доступа сотрудника: у данного сотрудника нет доступа к сервису|Пытаемся деактивировать сотрудника, который никогда не был активным.
|43020|Ошибка деактивации доступа сотрудника: данный сотрудник уже деактивирован|Нельзя деактивировать не активного сотрудника.
|43021|Ошибка деактивации доступа сотрудника: нельзя деактивировать свою учетную запись|Нельзя деактивировать свою учетную запись.
|43022|Ошибка активации доступа сотрудника: логин пользователя '{логин пользователя}' уже используется|При активации сотрудника указан логин, который уже используется у другого сотрудника.
...
|43024|Ошибка активации доступа сотрудника: на аккаунте максимальное число активных сотрудников|По тарифу уже максимальное число активных сотрудников.
...
|43026|Ошибка активации доступа сотрудника: у пользователя нет или указан не верный email|У сотрудника нет или указан не валидный email.
|43027|"Ошибка изменения доступа сотрудника: активировать, деактивировать доступ сотрудника к сервису или сбрасывать пароль может только администратор|Изменять активность может только администратор.
