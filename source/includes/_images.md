...
у них совпадало `filename` и `context`. У одинаковых Изображений одинаковое значение параметра `id`. 
...
- **rows** - Массив JSON объектов, представляющих собой [Изображения](#изображение-изображения).
...
Результат - JSON представление созданного Изображения. Для создания и добавления новго Изображения к Товару или Комплекту, 
необходимо и достаточно указать `id` сущности, к которой добавляется Изображение, в url запроса и указать не пустые поля 
`filename` и `context` в переданном объекте.

В поле `context` нужно указать изображение, закодированное в Base64, в поле `filename` - имя файла с расширением.
...
Успешный запрос. Результат - JSON представление всех картинок Товара.
...
### Обновление Изображений у Товара или Комплекта
...
> Пример обновления Изображений у Товара
...
```json
{
    ...
    "id": "4e2cc881-22cc-11e9-ac12-000c000000a5",
