...
+ **article** - параметр для фильтрации по артикулам товаров. Можно использовать операторы `=`, `!=`, `~`, `~=`, `=~`. Можно передать несколько значений. Можно указать пустое значение.
...
+ **updated** - параметр для фильтрации по времени последнего обновления сущностей. Можно использовать операторы `=`, `<=`, `>=`. Передается в виде строки в [формате дата-время](../#mojsklad-json-api-obschie-swedeniq-format-daty-i-wremeni).
...
+ **archived** - параметр для фильтрации по архивности товаров. Возможные значения: true, false. Для выдачи как архивных, так и не архивных товаров нужно передать сразу два значения true и false. По умолчанию в выдачу попадают только не архивные товары.
...
+ **alcoholic** -  параметр для фильтрации по признаку алкогольного товара. Возможные значения: true, false.
+ **alcoholic.type** - параметр для фильтрации по коду ЕГАИС товаров. Можно использовать операторы `=` и `!=`. Значение параметра - целое число. Можно передать пустое значение, тогда в выборку попадут товары с незаполненным или заполненным кодом ЕГАИС.
+ **isSerialTrackable** -  параметр для фильтрации по признаку товара с учетом серийных номеров. Возможные значения: true, false.
...
+ **search** - префиксный поиск по строковым полям, выводимым в ассортименте. Для данного параметра нужно использовать оператор `=`. Поиск по штрихкодам выполняется по полному соотвествию. Можно передать только одно значение.
...
+ **type** - параметр для фильтрации по типу сущностей. Возможные значения `product`, `service`, `bundle`, `variant` и `consignment`. В выдачу попадут сущности только указанных типов. Для данного параметра нужно использовать оператор `=`. Можно передать несколько значений.
 
При использовании фильтров **article**, **alcoholic**, **alcoholic.type**, **weighed**, **isSerialTrackable** и фильтров **stockMode**, **quantityMode** со значениями отличными от all в выдачу не попадают услуги и комлекты.
...
- `filter=alcoholic=true`
- `filter=alcoholic.type=123`
- `filter=isSerialTrackable=true`
...
- `filter=search=див`
- `filter=type=product;type=variant`
