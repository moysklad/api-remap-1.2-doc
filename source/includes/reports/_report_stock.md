# Отчеты
## Отчет Остатки
Отчет об остатках в МоемСкладе доступен в расширенном и кратком виде. Запросить отчет можно с помощью JSON API или вебхуков на изменение остатков. Используйте JSON API, если остатки меняются часто, и вы хотите запрашивать их каждые несколько минут. Если остатки меняются реже, чем раз в несколько минут, и вы хотите получать уведомления об изменениях, используйте вебхуки. 

Для работы с отчетом дополнительных прав не требуется. 

На содержание отчета влияют настройки видимости пользователя, который запросил отчет. При отсутствии соответствующих прав видимости в отчете могут не отображаться: 
...
| **folder**       | Object                                                    | Группа Товара/Модификации/Cерии. [Подробнее тут](../reports/#otchety-otchet-ostatki-vse-ostatki-gruppa)<br>`+Обязательное при ответе` |
...
| **uom**          | Object                                                    | Единица измерения. [Подробнее тут](../reports/#otchety-otchet-ostatki-vse-ostatki-edinica-izmereniq)<br>`+Обязательное при ответе`    |
...
| **quantityMode**   | Enum        | `=`        | параметр для фильтрации по значению доступно. Значение по умолчанию nonEmpty. [Доступные значения](../reports/#otchety-otchet-ostatki-vse-ostatki-atributy-dostupnye-dlq-fil-tracii-dostupnye-znacheniq-dlq-quantitymode)                                                              |
...
| **stockMode**      | Enum        | `=`        | параметр для фильтрации по значению остатка. Значение по умолчанию all. [Доступные значения](../reports/#otchety-otchet-ostatki-vse-ostatki-atributy-dostupnye-dlq-fil-tracii-dostupnye-znacheniq-dlq-stockmode)                                                                       |
