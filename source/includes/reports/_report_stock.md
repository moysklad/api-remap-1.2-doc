...
Параметр `withRecalculate` доступен только на эндпоинте `/report/stock/bystore/current`.
По умолчанию выводятся только результаты с актуальным значением остатка.
Для добавления в вывод остатков, которые на данный момент пересчитывются, необходимо добавить параметр `withRecalculate=true`, значения таких остатков будут отображаться как null.
