...
Некоторые запросы могут приходиться на моменты пересчётов остатков. Чаще всего это связано с массовыми изменениями документов на больших аккаунтах.
Строки с неактуальными остатками не отображаются в отчёте, что может по-разному повлиять на интеграцию:

- если интерграция подписана на хук на остатки, хук придёт по окончании пересчёта
- если используется параметр `changedSince`, в интервал попадают товары, пересчёт по которым закончился в интервале
- если отсутствующие в отчёте товары не участвуют в синхронизации, во внешней системе остатки останутся старыми до окончания пересчёта, затем поменяются на актуальные
- если интеграция отрабатывает отсутствие значения аналогично значению 0, можно запросить отчёт с `include=invalid` для отображения валидности (остаток будет только для валидных строк)
...
> Запрос на получение текущих остатков с невалидными строками.

```shell
curl -X GET
  "https://api.moysklad.ru/api/remap/1.2/report/stock/all/current?include=invalid"
  -H "Authorization: Basic <Credentials>"
  -H "Accept-Encoding: gzip"
```
...
```json
[
  {"assortmentId":"12345678-5838-aaeb-0a80-003a003ef439","stock":11,"isValid":true},
  {"assortmentId":"12345678-279c-aaeb-0a80-00d6001f847c","stock":46.5,"isValid":true},
  {"assortmentId":"12345678-6ac4-aaeb-0a80-156fa48cd135","isValid":false}
]
```
