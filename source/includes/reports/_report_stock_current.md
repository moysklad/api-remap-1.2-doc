...
- Не стоит запрашивать часто с большим интервалом (пример: каждые пять минут с интервалом последние сутки). Использовать по формуле не превышающей `2 * интервал` раз (пример: каждую минуту запрашивать остатки за прошедшие две минуты)
- Рекомендуется проводить полную синхронизацию остатков раз в сутки и более, в зависимости от частоты изменения остатков.
