...
Список запросов, для которых реализована возможность работы в асинхронном режиме, вы можете посмотреть в [документации](#/async#2-asinhronnyj-obmen).
...
При большом количестве номенклатуры и складов, ранее необходимо было запрашивать [отчет остатков по складам](#/reports/report-stock#3-poluchit-ostatki-po-skladam) 
...
Для того, чтобы не опрашивать эндпоинт статуса выполняемой асинхронной задачи можно настроить [вебхук](#/dictionaries/webhook#2-vebhuki) на оповещение, когда задача будет завершена.
