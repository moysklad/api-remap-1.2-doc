...
### Новое упоминание в комментариях Задач
...
### Новое сообщение от менеджера отдела продаж

SalesManagerChatMessage - уведомление о новом сообщении от менеджера отдела продаж

#### SalesManagerChatMessage: Атрибуты уведомления
...
| **created**     | DateTime                       | Дата и время формирования Уведомления<br>`+Обязательное при ответе` `+Только для чтения`                                                             |
| **read**        | Boolean                        | Признак того, было ли Уведомление прочитано<br>`+Обязательное при ответе` `+Только для чтения`                                                       |
| **title**       | String(255)                    | Заголовок уведомления. Формат: «Новое сообщение от {имя и фамилия менеджера}»<br>`+Обязательное при ответе` `+Только для чтения`                     |
| **description** | String(4096)                   | Текст сообщения от менеджера отдела продаж<br>`+Обязательное при ответе` `+Только для чтения` |

**Параметры**

| Параметр | Описание                                                                            |
|:---------|:------------------------------------------------------------------------------------|
| **id**   | `string` (required) *Example: 4c87e5eb-97b2-11f1-0a81-08ec00000003* id Уведомления. |

> Запрос на получение Уведомления с указанным id.

```shell
curl --compressed -X GET \
  "https://api.moysklad.ru/api/remap/1.2/notification/4c87e5eb-97b2-11f1-0a81-08ec00000003" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip"
```
...
{
  "meta": {
    "href": "https://api.moysklad.ru/api/remap/1.2/notification/4c87e5eb-97b2-11f1-0a81-08ec00000003",
    "type": "SalesManagerChatMessage",
    "mediaType": "application/json"
  },
  "id": "4c87e5eb-97b2-11f1-0a81-08ec00000003",
  "accountId": "3a0c5979-f5ab-11ee-c0a8-300f00000001",
  "created": "2026-08-14 10:32:26.913",
  "read": false,
  "title": "Новое сообщение от Льва Кошкина",
  "description": "Добрый день! Готов ответить на ваши вопросы."
}
```
