...
По Элементам пользовательского справочника можно осуществлять контекстный поиск с помощью специального параметра `search`. Подробнее можно узнать по [ссылке](#/general#3-kontekstnyj-poisk).

Поиск среди объектов Элементов пользовательского справочника на соответствие поисковой строке будет осуществлен по следующим полям:
