# Сущности
## Ассортимент (Deprecated)
Сущность assortment представляет собой список всех товаров, услуг, комплектов, модификаций и партий с полями `stock`,
...
**Внимание**: Данный эндпоинт в 2027 году будет постепенно выведен из эксплуатации. Рекомендуется переходить на [новый эндпоинт ассортимента](#/dictionaries/assortment#2-assortiment), который работает быстрее и активно развивается.
...
| **article**           | параметр для фильтрации по артикулам товаров и комплектов. Можно использовать операторы `=`, `!=`, `~`, `~=`, `=~`. Можно передать несколько значений. Можно указать пустое значение. При фильтрации используется артикул головного товара. Модификации головного товара включаются в результат независимо от их собственных артикулов.                                                                                                                                                             |
...
| **quantityMode**      | параметр для фильтрации по значению доступно. Значение по умолчанию all. [Доступные значения](#/dictionaries/assortment-legacy#5-dostupnye-znacheniya-dlya-quantitymode)                                                                                                                                      |
| **search**            | префиксный поиск по строковым полям, выводимым в ассортименте. Для данного параметра нужно использовать оператор `=`. Поиск по штрихкодам выполняется по полному соотвествию. Можно передать только одно значение.[Подробнее тут](#/dictionaries/assortment-legacy#5-dostupnye-znacheniya-dlya-search)        |
| **shared**            | параметр для фильтрации по признаку общего доступа. Возможные значения: true, false.                                                                                                                                                                                                                                                               |
| **stockMode**         | параметр для фильтрации по значению остатка. Значение по умолчанию all. [Доступные значения](#/dictionaries/assortment-legacy#5-dostupnye-znacheniya-dlya-stockmode)                                                                                                                                          |
| **stockMoment**       | момент времени, на который нужно вывести остатки. Передается в виде строки в [формате дата-время](#/general#3-format-daty-i-vremeni)                                                                                                                                                                                    |
...
| **updated**           | параметр для фильтрации по времени последнего обновления сущностей. Можно использовать операторы `=`, `<`, `<=`, `>`, `>=`. Действие строгих операторов синонимично нестрогим. Передается в виде строки в [формате дата-время](#/general#3-format-daty-i-vremeni).                                                      |
...
| **доп. поле(url)**    | параметром фильтрации служит url дополнительного поля. Оператор фильтрации зависит от типа доп. поля. [Подробнее тут](#/general#5-dostupnye-operatory-dlya-filtracii-dop-polej).                                                     |

Также доступна фильтрация по доп. полям. Подробнее про фильтрацию по доп. полям можно посмотреть в соответствующем [разделе](#/general#4-filtraciya-po-dopolnitelnym-polyam).
...
Для данного параметра нужно использовать оператор `=`. Поиск по штрихкодам выполняется по полному соотвествию. Можно передать только одно значение.
   ...
   + по штрихкоду фасовок товаров **barcode**  
   + по штрихкоду фасовок модификаций **barcode**
...
| **uniqueCodeRules** | Object                                                    | Настройки уникальности кода для сущностей справочника. [Подробнее тут](#/dictionaries/assortment-legacy#5-nastrojki-unikalnosti-koda-dlya-sushnostej-spravochnika)<br>`+Обязательное при ответе`                       |
| **barcodeRules**    | Object                                                    | Настройки правил штрихкодов для сущностей справочника. [Подробнее тут](#/dictionaries/assortment-legacy#5-nastrojki-pravil-shtrihkodov-dlya-sushnostej-spravochnika)<br>`+Обязательное при ответе`                     |
