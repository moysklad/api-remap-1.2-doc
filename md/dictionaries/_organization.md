...
| **actualAddressFull**      | Object                         |                            | Фактический адрес Юрлица с детализацией по отдельным полям. [Подробнее тут](#/dictionaries/organization#5-adres)                                                                                    |
...
| **companyType**            | Enum                           | `=` `!=`                   | Тип Юрлица . В зависимости от значения данного поля набор выводимых реквизитов контрагента может меняться. [Подробнее тут](#/dictionaries/organization#4-tip-yurlica)<br>`+Обязательное при ответе` |
...
| **meta**                   | [Meta](#/general#3-metadannye) |                            | Метаданные Юрлица<br>`+Обязательное при ответе`                                                                                                                                                     |
...
| **advancePaymentVat**       | Float         |                                                                                                                                                   | Налоговая ставка для авансов для плательщиков НДС. Можно использовать значение только из существующих [ставок НДС](#/dictionaries/taxrate#3-poluchit-stavki-nds) |
| **attributes**              | Array(Object) | [Операторы доп. полей](#/general#4-filtraciya-po-dopolnitelnym-polyam) | Массив метаданных дополнительных полей Юрлица                                                                                                                                  |
| **certificateDate**         | DateTime      |                                                                                                                                                   | Дата свидетельства <br>`+RU`                                                                                                                                                   |
| **certificateNumber**       | String(255)   |                                                                                                                                                   | Номер свидетельства <br>`+RU`                                                                                                                                                  |
| **chiefAccountSign**        | Object        |                                                                                                                                                   | Подпись главного бухгалтера. [Подробнее тут](#/dictionaries/organization#5-podpisi-i-pechat)                                     |
...
| **directorSign**            | Object        |                                                                                                                                                   | Подпись руководителя. [Подробнее тут](#/dictionaries/organization#5-podpisi-i-pechat)                                            |
| **email**                   | String(255)   | `=` `!=` `~` `~=` `=~`                                                                                                                            | Адрес электронной почты                                                                                                                                                        |
| **fax**                     | String(255)   | `=` `!=` `~` `~=` `=~`                                                                                                                            | Номер факса                                                                                                                                                                    |
| **inn**                     | String(255)   | `=` `!=` `~` `~=` `=~`                                                                                                                            | ИНН <br>`+RU`                                                                                                                                                                  |
| **isEgaisEnable**           | Boolean       |                                                                                                                                                   | Включен ли ЕГАИС для данного Юрлица. Всегда false                                                                                                                                           |
...
| **mod\_\_requisites\_\_uz** | Object        |                                                                                                                                                   | Реквизиты Узбекистана. [Подробнее тут](#/dictionaries/organization#5-rekvizity-uzbekistana) <br>`+UZ`                                            |
...
Подробнее о типах юридических лиц [тут](#/dictionaries/organization#4-tip-yurlica).
...
Если тип Юрлица `Юридическое лицо, Казахстан`, будут выведены следующие поля реквизитов:
...
| **mod\_\_requisites\_\_kz.oked**                 | ОКЭД                                      |
...
Если тип Юрлица `Индивидуальный предприниматель, Казахстан`, будут выведены следующие поля реквизитов:
...
Если тип Юрлица `Физическое лицо, Казахстан`, будут выведены следующие поля реквизитов:
...
О работе с доп. полями юрлиц можно прочитать [здесь](#/general#3-rabota-s-dopolnitelnymi-polyami)
...
> Пример создания Юрлица с расчетными счетами и указанием валюты счетов.

```shell
  curl --compressed -X POST \
    "https://api.moysklad.ru/api/remap/1.2/entity/organization?expand=accounts" \
    -H "Authorization: Basic <Credentials>" \
    -H "Accept-Encoding: gzip" \
    -H "Content-Type: application/json" \
      -d '{  
...
{
    "meta": {
        "href": "https://api.moysklad.ru/api/remap/1.2/entity/organization/13033b66-4a17-11f0-0a83-01ea00000020?expand=accounts",
        "metadataHref": "https://api.moysklad.ru/api/remap/1.2/entity/organization/metadata",
        "type": "organization",
        "mediaType": "application/json",
        "uuidHref": "https://online.moysklad.ru/app/#mycompany/edit?id=13033b66-4a17-11f0-0a83-01ea00000020"
    },
    "id": "13033b66-4a17-11f0-0a83-01ea00000020",
    "accountId": "5e259af8-4571-11f0-0a82-19ae00000023",
    "owner": {
        "meta": {
            "href": "https://api.moysklad.ru/api/remap/1.2/entity/employee/5e8379dc-4571-11f0-0a81-141c00000520",
            "metadataHref": "https://api.moysklad.ru/api/remap/1.2/entity/employee/metadata",
            "type": "employee",
            "mediaType": "application/json",
            "uuidHref": "https://online.moysklad.ru/app/#employee/edit?id=5e8379dc-4571-11f0-0a81-141c00000520"
        }
    },
    "shared": true,
    "group": {
        "meta": {
            "href": "https://api.moysklad.ru/api/remap/1.2/entity/group/5e261def-4571-11f0-0a82-19ae00000024",
            "metadataHref": "https://api.moysklad.ru/api/remap/1.2/entity/group/metadata",
            "type": "group",
            "mediaType": "application/json"
        }
    ...
    "accounts": {
        "meta": {
            "href": "https://api.moysklad.ru/api/remap/1.2/entity/organization/13033b66-4a17-11f0-0a83-01ea00000020/accounts",
            "type": "account",
            "mediaType": "application/json",
            "size": 1,
            "limit": 1000,
        ...
        "rows": [
            {
                "meta": {
                    "href": "https://api.moysklad.ru/api/remap/1.2/entity/organization/13033b66-4a17-11f0-0a83-01ea00000020/accounts/130353db-4a17-11f0-0a83-01ea00000021",
                    "type": "account",
                    "mediaType": "application/json"
                },
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) юрлиц.
...
| Название         | Тип                                                       | Описание                                                                                                   |
| ---------------- | :-------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------- |
| **meta**         | [Meta](#/general#3-metadannye) | Ссылка на метаданные юрлиц                                                                                 |
| **attributes**   | Array(Object)                                             | Массив объектов доп. полей юрлиц в формате [Метаданных](#/general#3-metadannye) |
...
- если при создании Юрлица поле **accounts** отсутствовало и счета добавляются впервые, хотя бы один из добавляемых расчетных счетов должен быть в валюте учета компании, иначе вернется ошибка;
- если при создании Юрлица поле **accounts** присутствовало и добавляются дополнительные расчетные счета, дополнительные счета можно создавать в любой валюте;
- после создания расчетного счета валюту (поле `currency`) изменить невозможно — при попытке изменить метаданные валюты любого из счетов вернется ошибка.
...
**Важно:** После создания расчетного счета его валюта больше не может быть изменена. При попытке изменить метаданные валюты в поле `currency` будет возвращена ошибка.
...
#### Описание изменение счета Юрлица
Обновляются счета Юрлица с указанным id.
Обновляются все поля, указанные в JSON объекте запроса, кроме
помеченных `Только для чтения` в описании [атрибутов счетов Юрлица](#/dictionaries/organization#3-zaprosy-scheta-yurlica).
Поля, которые не были указаны в JSON запроса, не изменяются.

**Важно:** Для существующих расчетных счетов запрещено менять валюту (поле `currency`) — при попытке изменить метаданные валюты любого из счетов вернется ошибка. Дополнительные счета можно создавать в любой валюте.
...
| :------------ | :----------------------------------------------------------------------------- |
| **id**        | `string` (required) *Example: 7944ef04-f831-11e5-7a69-971500188b19* id Юрлица. |
| **positionID** | `string` (required) *Example: d9560d0e-6703-11e7-9464-e4de00000052* id счета.  |

> Запрос на удаление счета Юрлица с указанным id.

```shell
curl --compressed -X DELETE \
  "https://api.moysklad.ru/api/remap/1.2/entity/organization/7944ef04-f831-11e5-7a69-971500188b19/accounts/d9560d0e-6703-11e7-9464-e4de00000052" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip"
```
...
В теле запроса нужно передать массив, содержащий JSON метаданных счетов Юрлица, которые вы хотите удалить.

> Запрос на массовое удаление счетов Юрлица.

```shell
curl --compressed -X POST \
  "https://api.moysklad.ru/api/remap/1.2/entity/organization/7944ef04-f831-11e5-7a69-971500188b19/accounts/delete" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip" \
  -H "Content-Type: application/json" \
  -d '[
        ...
        {
          "meta": {
            "href": "https://api.moysklad.ru/api/remap/1.2/entity/organization/7944ef04-f831-11e5-7a69-971500188b19/accounts/97fc6499-b96f-11e7-9464-e4de00000006",
            "type": "account",
            "mediaType": "application/json"
          }
        }
