...
| **agent**                | [Meta](#/general#3-metadannye) | Метаданные юрлица<br>`+Обязательное при ответе`                                        |
...
- хотя бы один из переданных расчетных счетов должен быть в валюте учета компании; при отсутствии такого счета создание завершится ошибкой (#/errors#3-kody-oshibok-dlya-raschetnyh-schetov);
- после создания расчетного счета его валюту (поле `currency`) изменить нельзя — при попытке изменить метаданные валюты счета сервер вернет ошибку (#/errors#3-obshie-oshibki-validacii).
...
- если при создании юрлица поле **accounts** отсутствовало и счета добавляются впервые, хотя бы один из добавляемых расчетных счетов должен быть в валюте учета компании, иначе вернется ошибка (#/errors#3-kody-oshibok-dlya-raschetnyh-schetov);
...
- после создания расчетного счета валюту (поле `currency`) изменить невозможно — при попытке изменить метаданные валюты любого из счетов вернется ошибка (#/errors#3-obshie-oshibki-validacii).
...
**Важно:** После создания расчетного счета его валюта больше не может быть изменена. При попытке изменить метаданные валюты в поле `currency` будет возвращена ошибка (#/errors#3-obshie-oshibki-validacii).
...
**Важно:** Для существующих расчетных счетов запрещено менять валюту (поле `currency`) — при попытке изменить метаданные валюты любого из счетов вернется ошибка (#/errors#3-obshie-oshibki-validacii). Дополнительные счета можно создавать в любой валюте.
