 ...
 **Важно:** у организации, передаваемой в поле `organization` при создании и обновлении точки продаж, **основной расчётный счёт** должен быть в **валюте учёта** компании. Иначе при сохранении вернётся ошибка [4004](#/errors#3-kody-oshibok-dlya-tochek-prodazh).
...
| **addressFull**                         | Object                         |                            | Адрес с детализацией по отдельным полям. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-adres)                                                                                                              |
...
| **cashiers**                            | [Meta](#/general#3-metadannye) |                            | Метаданные Кассиров<br>`+Обязательное при ответе` `+Expand`                                                                                                                                                                  |
...
| **environment**                         | Object                         |                            | Информация об окружении. [Подробнее тут](#/dictionaries/retailstore#5-okruzhenie)<br>`+Обязательное при ответе` `+Только для чтения`                                                                                         |
| **externalCode**                        | String(255)                    | `=` `!=` `~` `~=` `=~`     | Внешний код Точки продаж<br>`+Обязательное при ответе` `+Только для чтения`                                                                                                                                                  |
| **filterAgentsTags**                    | Array(String)                  |                            | Коллекция групп покупателей, представленных в формате строк. Определяет группы, из которых выгружаются покупатели. Значения `null` игнорируются                                                                              |
| **fiscalType**                          | Enum                           |                            | Тип формирования чеков. [Подробнее тут](#/dictionaries/retailstore#5-tip-formirovaniya-chekov)<br>`+Обязательное при ответе`                                                                                                 |
...
| **lastOperationNames**                  | Array(Object)                  |                            | Последние операции. [Подробнее тут](#/dictionaries/retailstore#5-poslednie-operacii)<br>`+Обязательное при ответе` `+Только для чтения` `+Устаревшее`                                                                        |
| **markingSellingMode**                  | Enum                           |                            | Режим продажи маркированной продукции, если используется формат фискальных документов версии 1.2. [Подробнее тут](#/dictionaries/retailstore#5-prodazha-markirovannyh-tovarov)<br>`+Обязательное при ответе`                 |
| **marksCheckMode**                      | Enum                           |                            | Настройка проверки КМ перед продажей в ГИС МТ (по умолчанию CORRECT_MARKS_ONLY) [Подробнее тут](#/dictionaries/retailstore#5-nastrojka-proverki-km-pered-prodazhej-v-gis-mt)<br>`+Обязательное при ответе`                   |
| **masterRetailStores**                  | MetaArray                      |                            | Коллекция метаданных точек продаж, которые могут фискализировать операции с текущей точки продаж, если `minionToMaster` = `CHOSEN`<br>`+Expand`                                                                              |
| **meta**                                | [Meta](#/general#3-metadannye) |                            | Метаданные Точки продаж<br>`+Обязательное при ответе`                                                                                                                                                                        |
| **minionToMasterType**                  | Enum                           |                            | Стратегия выбора кассы для фискализации облачных чеков. [Подробнее тут](#/dictionaries/retailstore#5-strategiya-vybora-kassy-dlya-fiskalizacii-oblachnyh-chekov)<br>`+Обязательное при ответе`                               |
...
| **priorityOfdSend**                     | Enum                           |                            | Приоритет отправки электронного чека. Активен только, когда отправка электронных чеков через ОФД включена. [Подробнее тут](#/dictionaries/retailstore#5-prioritet-otpravki-elektronnogo-cheka)<br>`+Обязательное при ответе` |
| **productFolders**                      | MetaArray                      |                            | Коллекция Метаданных групп товаров, из которых можно выгружать товары<br>`+Expand`                                                                                                                                           |
| **qrAcquire**                           | [Meta](#/general#3-metadannye) |                            | Метаданные Банка-эквайера по операциям по QR-коду<br>`+Expand`                                                                                                                                                               |
...
| **receiptTemplate**                     | [Meta](#/general#3-metadannye) |                            | Метаданные шаблона печати кассовых чеков<br>`+Expand`                                                                                                                                                                        |
...
| **state**                               | Object                         |                            | Информация статусе точки продаж. [Подробнее тут](#/dictionaries/retailstore#5-atributy-sushnosti-status)<br>`+Только для чтения`                                                                                             |
| **store**                               | [Meta](#/general#3-metadannye) | `=` `!=`                   | Метаданные Склада<br>`+Обязательное при ответе` `+Expand` `+Необходимо при создании`                                                                                                                                         |
| **tobaccoMrcControlType**               | Enum                           |                            | Контроль МРЦ для табачной продукции. [Подробнее тут](#/dictionaries/retailstore#5-tip-kontrolya-mrc-dlya-tabachnoj-produkcii)<br>`+Обязательное при ответе`                                                                  |
...
##### Настройка проверки КМ перед продажей в ГИС МТ:

| Название                    | Описание                                                                                   |
| --------------------------- |:-------------------------------------------------------------------------------------------|
| **CORRECT_MARKS_ONLY**      | Продажа маркированных товаров: только с правильными кодами маркировки                      |
| **WITHOUT_ERRORS**          | Продажа маркированных товаров: с правильными кодами и те, которые не удалось проверить     |
| **ALL**                     | Продажа маркированных товаров: все — независимо от результатов проверки кодов              |
| **ALL_CHECKED**             | Продажа маркированных товаров: все проверенные, с правильными и ошибочными кодами          |
...
| **software**        | Object      | Информация о ПО. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-po) |
| **chequePrinter**   | Object      | Данные о ККТ. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-kkt)   |
...
| Название              | Тип         | Описание                                                                                                                                                                                |
| --------------------- | :---------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **driver**            | Object      | Информация об используемом драйвере. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-drajver)             |
| **firmwareVersion**   | String(255) | Версия прошивки ККТ                                                                                                                                                                     |
| **fiscalDataVersion** | String(255) | Формат фискальных данных                                                                                                                                                                |
| **fiscalMemory**      | Object      | Информация о фискальном накопителе. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-fiskalnyj-nakopitel) |
| **name**              | String(255) | Наименование ПО<br>`+Обязательное при ответе` `+Необходимо при создании`                                                                                                                |
...
| Название            | Тип      | Описание                                                                                                                                                                                                               |
| ------------------- | :------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **sync**            | Object   | Состояние синхронизации. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-sinhronizaciya)                                 |
| **lastCheckMoment** | DateTime | Дата и время последней синхронизации                                                                                                                                                                                   |
| **fiscalMemory**    | Object   | Информация о фискальном накопителе. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-fiskalnaya-pamyat)                   |
| **paymentTerminal** | Object   | Информация о платежном терминале. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-platezhnyj-terminal)<br>`+Устаревшее` |
...
| Название            | Тип      | Описание                                                                                                                                                                         |
| ------------------- |:---------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **error**           | Object   | Информация об ошибке ФН. [Подробнее тут](#/dictionaries/retailstore#6-atributy-sushnosti-oshibka) |
...
**Поле считается устаревшим и будет удалено в ближайшее время**
...
##### Кассиры
Сущность кассир представляет собой объект, содержащий ссылки на [Сотрудника](#/dictionaries/employee#2-sotrudnik), назначенного кассиром,
и [Точку продаж](#/dictionaries/retailstore#2-tochka-prodazh), к которой он привязан.
Подробнее о сущности Кассира можно посмотреть в разделе [Кассиры](#/dictionaries/cashier#2-kassir)
...
| **meta**        | [Meta](#/general#3-metadannye) | Метаданные Кассира<br>`+Обязательное при ответе`                                                                       |
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) точек продаж.
