...
| **quantity** | Float                                                     | Количество Товаров в упаковке данного вида<br>`+Обязательное при ответе`                                                                                                                                              |
...
В URL указывается API ID товара (поле `id` в ответе). При указании UUID из интерфейса (значение `id` в `meta.uuidHref`) сервер вернёт редирект **308** с заголовком `Location` на URL с API ID. [Подробнее](#/general#3-obrabotka-oshibok)
