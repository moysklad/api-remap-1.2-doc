## Изображение
### Изображения
Средствами JSON API можно создавать и обновлять сведения по Изображениям для Товаров, Комплектов, Модификаций и Карточек контента, запрашивать списки Изображений, 
а также сведения по отдельным Изображениям.
Кодом сущности для Изображения в составе JSON API является ключевое слово **image**.

Товары, Комплекты, Модификации и Карточки контента могут содержать множество одинаковых Изображений. Изображения считаются одинаковыми, если при добавлении Изображений 
у них совпадало `filename` и `content`. У одинаковых Изображений одинаковое значение параметра `id`. 
У Товара, Комплекта, Модификации может быть не более 10 Изображений. 
У Карточки контента не более 15 Изображений.
...
### Получить список Изображений Товара, Комплекта, Модификации или Карточки контента
Запрос на получение всех Изображений Товара, Комплекта, Модификации или Карточки контента для данной учетной записи.
...
| **rows**    | Array(Object)                                             | Массив JSON объектов, представляющих собой [Изображения](#/dictionaries/images#2-izobrazhenie). |
...
Аналогично [товару](#/dictionaries/product#3-poluchit-tovar): при указании UUID из интерфейса (или из `meta.uuidHref`) вместо API ID (поле `id` в ответе) сервер вернёт редирект **308** с заголовком `Location` на URL `.../entity/product/{id}/images`.

> Получить список Изображений для Товара

```shell
curl --compressed -X GET \
  "https://api.moysklad.ru/api/remap/1.2/entity/product/7944ef04-f831-11e5-7a69-971500188b19/images" \
...
```json
{
    ...
    "rows": [
        ...
        {
            "meta": {
                "href": "https://api.moysklad.ru/api/remap/1.2/entity/product/7944ef04-f831-11e5-7a69-971500188b19/images/933e41ac-1946-4bf0-9b21-51f2051f3e9f",
                "type": "image",
                "mediaType": "application/json",
...
#### Получить постоянную ссылку на Изображение Товара, Комплекта, Модификации или Карточки контента
...
на вывод поля meta у Изображения, поэтому его можно применять вместе с параметром `expand=images`. [Подробнее о параметре fields](#/general#3-chto-takoe-fields).
...
### Получить временную ссылку на Изображение Товара, Комплекта, Модификации или Карточки контента
...
Успешный запрос. Результат - пустой JSON

```json
```

Ссылка на скачивание изображения находится в заголовке `location`. Получить изображение по ссылке можно без авторизации.
Время активности ссылки составляет 1 минуту.

### Добавить Изображение к Товару, Комплекту, Модификации или Карточке контента
Добавить новое Изображение к Товару, Комплекту, Модификации или Карточке контента.
...
Результат - JSON представление обновленного списка Изображений. Для создания и добавления нового Изображения к Товару, Комплекту, Модификации или Карточке контента, 
...
### Изменение списка Изображений у Товара, Комплекта, Модификации или Карточки контента

В теле запроса нужно передать массив, содержащий JSON представления Изображений, которые вы хотите установить для Товара, Комплекта, Модификации или Карточки контента.
Если необходимо оставить некоторые Изображения у Товара, Комплекта, Модификации или Карточки контента, то тело запроса должно содержать их идентификаторы в виде метаданных.
...
При удалении изображения удаляется первое найденное с данным идентификатором изображение у Товара, Комплекта, Модификации или Карточки контента.
...
curl --compressed -X DELETE \
  "https://api.moysklad.ru/api/remap/1.2/entity/product/7944ef04-f831-11e5-7a69-971500188b19/images/19f1edc0-fc42-4001-94cb-c9ec9c62ec10" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip"
```
> Response 200 (application/json)
...
При удалении нескольких изображений у Товара, Комплекта, Модификации или Карточки контента, удаляются первые найденые по `id` изображения, указанные в теле запроса.
...
```shell
curl --compressed -X POST \
  "https://api.moysklad.ru/api/remap/1.2/entity/product/7944ef04-f831-11e5-7a69-971500188b19/images/delete" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip" \
  -H "Content-Type: application/json" \
        -d '[  
  {  
    "meta":{  
      "href":"https://api.moysklad.ru/api/remap/1.2/entity/product/7944ef04-f831-11e5-7a69-971500188b19/images/1aadd77f-90f9-4be6-bede-373f350b0e03",
      "type":"image",
      "mediaType":"application/json",
      "downloadHref":"https://api.moysklad.ru/api/remap/1.2/download/1aadd77f-90f9-4be6-bede-373f350b0e03"
    }
  },
  {  
    "meta":{  
      "href":"https://api.moysklad.ru/api/remap/1.2/entity/product/7944ef04-f831-11e5-7a69-971500188b19/images/f6aaab17-65a0-4425-841b-277aeef5b089",
      "type":"image",
      "mediaType":"application/json",
      "downloadHref":"https://api.moysklad.ru/api/remap/1.2/download/f6aaab17-65a0-4425-841b-277aeef5b089"
    }
  }
]'
```
