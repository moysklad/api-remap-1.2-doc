...
| **minimumStock**       | Object                         |                            | Неснижаемый остаток. [Подробнее тут](#/dictionaries/variant#5-nesnizhaemyj-ostatok)<br>`+Выводится по запросу` |
| **name**               | String(255)                    | `=` `!=` `~` `~=` `=~`     | Наименование товара с Модификацией<br>`+Обязательное при ответе`                                                                                                          |
| **packs**              | Array(Object)                  |                            | фасовки модификации [Подробнее тут](#/dictionaries/variant#5-fasovki-modifikacii)                                                                                       |
...
| Название       | Тип                            | Описание                                                                                                                                                                                                   |
|----------------|:-------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **barcodes**   | Array(Object)                  | Массив штрихкодов фасовки модификации. Данный массив может содержать только один штрихкод. Для фильтрации по полю необходимо указывать его в единственном числе **barcode**.<br>`+Обязательное при ответе` |
| **id**         | UUID                           | ID фасовки модификации<br>`+Обязательное при ответе` `+Только для чтения`                                                                                                                                  |
| **parentpack** | [Meta](#/general#3-metadannye) | Метаданные родительской фасовки (фасовки товара), для которой переопределяется штрихкод<br>`+Обязательное при ответе` `+Expand`                                                                            |
