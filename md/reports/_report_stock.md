...
| **folder**       | Object                                                    | Группа Товара/Модификации/Партии. [Подробнее тут](#/reports/report-stock#4-gruppa)<br>                                                  |
| **image**        | [Meta](#/general#3-metadannye) | Метаданные изображения Товара/Модификации/Партии                                                                                                                                  |
| **inTransit**    | Float                                                     | Ожидание<br>`+Обязательное при ответе`                                                                                                                                           |
| **meta**         | [Meta](#/general#3-metadannye) | Метаданные Товара/Модификации/Партии по которой выдается остаток<br>`+Обязательное при ответе`                                                                                    |
...
| **consignment**    | Object      | `=` `!=`   | параметр для фильтрации по нескольким партиям. Значение параметра - ссылка на партию, которая должна быть включена в выборку или исключена из нее. Можно передать несколько значений. Данный параметр фильтрации можно комбинировать с параметрами `product` и `variant`.                           |
...
| **search**         | String(255) | `=` `!~`   | специальный параметр текстового поиска. Поиск осуществляется по вхождению подстроки в названия товаров, модификаций, партий.                                                                                                                                                                        |
...
| **groupBy**        | `string` (optional) тип, по которому нужно сгруппировать выдачу.По умолчанию параметр groupBy имеет значение variant. Если вы хотите увидеть объекты типа consignment, или только объекты типа product, необходимо выставить соответствующее значение параметра. <ul><li>groupBy=product - выдает только товары</li><li>groupBy=variant - выдает товары и модификации</li><li>groupBy=consignment - выдает товары, модификации, партии</li></ul>                                                                                                                                                         |
| **includeRelated** | `boolean` (optional) Вывод остатков по модификациям и партиям товаров. Параметр позволяет включить в выборку остатки по модификациям и партиям для товаров. Необходимым условием для применения параметра является **обязательное** наличие фильтрации по товарам или модификациям или их комбинации. При выбранном значении `includeRelated=true` будут включены все остатки для товаров, модификаций и партий, указанных в параметрах фильтрации.<br> При использовании параметра устанавливается параметр группировки `groupBy=consignment`, переданные значения для `groupBy` будут проигнорированы. |

Примеры использования параметра `includeRelated`: <br> `filter=variant!=<URL>&includeRelated=true` выводит остатки товаров, модификаций, партий за исключением конкретной модификации указанной в URL. <br>`filter=product=<URL>&includeRelated=true` выводит остатки конкретного товара указанного в URL, его модификации и партии.
...
|------------------|:-----|:-----------|---------------------------------------------------------------|
| **assortmentId** | UUID | `=`        | Выдать в отчёте только указанные товары, модификации и партии |
...
Отчет показывает остатки товара/модификации/партии в ячейках для отдельно указанного склада (складов) или для всех складов по отдельному товару (товарам).
...
| Название                       | Тип         | Фильтрация | Описание                                                                                                                                                        |
| ------------------------------ | :---------- |:-----------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **consignment**                | Object      | `=`        | ссылка на партию, по которой нужно произвести фильтрацию.                                                                                                       |
| **moment**                     | DateTime    | `=`        | момент времени, на который нужно вывести остатки. Передается в виде строки в [формате дата-время](#/general#3-format-daty-i-vremeni)                            |
...
#### Примеры запросов отчета Остатки по документам

> Запрос на получение текущих остатков по документам.
