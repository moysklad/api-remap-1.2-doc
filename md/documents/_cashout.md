...
- Если **noClosingDocs** установлено в `true`, к расходному ордеру нельзя привязать документы во взаиморасчетах (поле **operations** должно быть пустым).
  При попытке одновременно передать **noClosingDocs = true** и непустой **operations** возвращается ошибка **22003**.
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) Расходных ордеров.
...
| **attributes**                 | Массив объектов доп. полей Расходных ордеров в формате [Метаданных](#/general#3-metadannye) |
...
помечены `Только для чтения` в описании [атрибутов Расходного ордера ](#/documents/cashout#2-rashodnyj-order).
