...
| **demand**        | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные Отгрузки, связанной с Перемещением<br>`+Только для чтения` `+Expand`                                                                                                                         |
...
| **internalOrder** | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные Внутреннего заказа, связанного с Перемещением<br>`+Expand`                                                                                                                                   |
| **customerOrder** | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные Заказа покупателя, связанного с Перемещением<br>`+Expand`                                                                                                                                    |
| **meta**          | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные Перемещения<br>`+Обязательное при ответе``+Change-handler`                                                                                                                                   |
...
| **overhead**      | Object                         |                                                                                                                                                   | Накладные расходы. [Подробнее тут](#/documents/move#4-nakladnye-rashody). Если Позиции Перемещения не заданы, то накладные расходы нельзя задать<br>`+Update-provider` |
...
| **sourceStore**   | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные склада, с которого совершается перемещение<br>`+Обязательное при ответе` `+Expand` `+Необходимо при создании``+Change-handler` `+Update-provider`                                            |
| **state**         | [Meta](#/general#3-metadannye) | `=` `!=`                                                                                                                                          | Метаданные статуса Перемещения<br>`+Expand` `+Change-handler` `+Update-provider`                                                                                                                        |
| **sum**           | Float                          | `=` `!=` `<` `>` `<=` `>=`                                                                                                                        | Сумма Перемещения в копейках<br>`+Обязательное при ответе` `+Только для чтения``+Change-handler`                                                                                                        |
| **supply**        | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные Приемки, связанной с Перемещением<br>`+Только для чтения` `+Expand`                                                                                                                          |
| **syncId**        | UUID                           | `=` `!=`                                                                                                                                          | ID синхронизации. После заполнения недоступен для изменения                                                                                                                                             |
| **targetStore**   | [Meta](#/general#3-metadannye) |                                                                                                                                                   | Метаданные склада, на который совершается перемещение<br>`+Обязательное при ответе` `+Expand` `+Необходимо при создании``+Change-handler` `+Update-provider`                                            |
| **updated**       | DateTime                       | `=` `!=` `<` `>` `<=` `>=`                                                                                                                        | Момент последнего обновления Перемещения<br>`+Обязательное при ответе` `+Только для чтения``+Change-handler`                                                                                            |

Филтрация по **sourceStore** и **targetStore** работает с другими наименованиями, **store** и **agentStore** соответственно и операторами `=` `!=`.
...
| ---------------- |:------| :-------------------------------------------------------------------------------------------------------------------------- |
| **sum**          | Float | Сумма в копейках<br>`+Обязательное при ответе` `+Update-provider`                                                                |
...
| Название            | Описание                                                                                                                                   |
|---------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|
| **customerOrder**   | Ссылка на Заказ Покупателя, с которым связано это перемещение в формате [Метаданных](#/general#3-metadannye)                               |
| **demand**          | Ссылка на Отгрузку, с которой связано это перемещение в формате [Метаданных](#/general#3-metadannye)                                       |
| **internalOrder**   | Ссылка на Внутренний заказ, с которым связано это перемещение в формате [Метаданных](#/general#3-metadannye)                               |
| **supply**          | Ссылка на Приёмку, с которой связано это перемещение в формате [Метаданных](#/general#3-metadannye)                                        |
| **productionTasks** | Массив ссылок на связанные производственные задания в формате [Метаданных](#/general#3-metadannye)              |

#### Позиции Перемещения
Позиции Перемещения - это список товаров/модификаций/партий.
Объект позиции Перемещения содержит следующие поля:

| Название       | Тип                                                       | Описание                                                                                                                                                                                                                                                                   |
|----------------|:----------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **accountId**  | UUID                                                      | ID учетной записи<br>`+Обязательное при ответе` `+Только для чтения``+Change-handler`                                                                                                                                                                                     |
| **assortment** | [Meta](#/general#3-metadannye) | Метаданные товара/услуги/партии/модификации, которую представляет собой позиция<br>`+Обязательное при ответе` `+Expand``+Change-handler` `+Update-provider`                                                                                                                                   |
| **id**         | UUID                                                      | ID позиции<br>`+Обязательное при ответе` `+Только для чтения``+Change-handler`                                                                                                                                                                                            |
| **overhead**   | Int                                                       | Накладные расходы. [Подробнее тут](#/documents/move#4-nakladnye-rashody). Если Позиции Перемещения не заданы, то накладные расходы нельзя задать<br>`+Обязательное при ответе` `+Только для чтения`                                      |
| **pack**       | Object                                                    | Упаковка Товара. [Подробнее тут](#/dictionaries/product#5-upakovki-tovara)<br>`+Change-handler` `+Update-provider`                                                                                                                               |
...
| **sourceSlot** | [Meta](#/general#3-metadannye) | Ячейка на складе, с которого совершается перемещение. [Подробнее тут](#/dictionaries/store#3-yachejki-sklada)<br>`+Expand` |
| **targetSlot** | [Meta](#/general#3-metadannye) | Ячейка на складе, на который совершается перемещение. [Подробнее тут](#/dictionaries/store#3-yachejki-sklada)<br>`+Expand` |
...
С позициями можно работать с помощью [специальных ресурсов для управления позициями Перемещения](#/documents/move#3-upravlenie-poziciyami-peremesheniya),
...
О работе с доп. полями Перемещений можно прочитать [здесь](#/general#3-rabota-s-dopolnitelnymi-polyami)
...
+ **sourceStore** - Ссылка на склад, с которого совершается перемещение, в формате [Метаданных](#/general#3-metadannye)
+ **targetStore** - Ссылка на склад, на который совершается перемещение, в формате [Метаданных](#/general#3-metadannye)
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) Перемещений.
...
| **attributes**                 | Массив объектов доп. полей Перемещений в формате [Метаданных](#/general#3-metadannye) |
...
### Управление позициями Перемещения 
...
+ **assortment** - Ссылка на товар/услугу/партию/модификацию, которую представляет собой позиция.
  Также можно указать поле с именем **service**, **consignment**, **variant** в соответствии с тем,
  чем является указанная позиция. Подробнее об этом поле можно прочитать в описании [позиции Перемещения](#/documents/move#4-pozicii-peremesheniya)
