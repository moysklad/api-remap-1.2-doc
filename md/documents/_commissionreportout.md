...
| **meta**                  | [Meta](#/general#3-metadannye)                             |                                                                                                                                                   | Метаданные Выданного отчета комиссионера<br>`+Обязательное при ответе` `+Только для чтения`                                                                                                       |
...
| **rewardType**            | String(255)                                                |                                                                                                                                                   | Тип вознаграждения. Возможные значения приведены в таблице полей типа enum ниже                                                                                                                                                                                |
...
| **state**                 | [Meta](#/general#3-metadannye)                             | `=` `!=`                                                                                                                                          | Метаданные статуса Выданного отчета комиссионера<br>`+Expand`                                                                                                                                     |
| **sum**                   | Float                                                      | `=` `!=` `<` `>` `<=` `>=`                                                                                                                        | Сумма Выданного отчета комиссионера в копейках<br>`+Обязательное при ответе` `+Только для чтения`                                                                                                 |
...
Таблица полей типа enum, их значений на UI и их представление в JSON:

| Имя поля         | Возможные Значения       | Соответствующее значение в JSON | Значение по умолчанию |
|:-----------------|:-------------------------|:--------------------------------|:----------------------|
| **rewardType**   | Процент от суммы продажи | PercentOfSales                  |                       |
|                  | Не рассчитывать          | None                            | Не рассчитывать       |
...
Позиции Выданного отчета комиссионера - это список товаров/модификаций/партий.
...
С позициями можно работать с помощью [специальных ресурсов для управления позициями Выданного отчета комиссионера](#/documents/commissionreportout#3-poluchit-vydannye-otchety-komissionera),
...
### Запросы - Выданный отчет комиссионера
...
помечены `Только для чтения` в описании [атрибутов Выданного отчета комиссионера](#/documents/commissionreportout#2-vydannyj-otchet-komissionera).
...
### Управление позициями Выданного отчета комиссионера 
...
Подробнее об этом поле можно прочитать в описании [позиции отчета комиссионера](#/documents/commissionreportout#4-pozicii-vydannogo-otcheta-komissionera)
