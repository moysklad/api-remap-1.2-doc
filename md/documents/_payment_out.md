...
| **noClosingDocs**      | Boolean                                                   |                                                                                                                                                    | Признак "Без закрывающих документов". <br>`+Обязательное при ответе`                                                                                              |
...
| Название       | Описание                                                                                                                                  |
|----------------|:------------------------------------------------------------------------------------------------------------------------------------------|
| **factureIn**  | Ссылка на Счет-фактуру полученный, с которым связан этот платеж в формате [Метаданных](#/general#3-metadannye) |
| **operations** | Массив ссылок на связанные операции в формате [Метаданных](#/general#3-metadannye)                             |
...
| **agent**                      | Ссылка на контрагента, сотрудника или юр.лицо в формате [Метаданных](#/general#3-metadannye) |
| **expenseItem**                | Статья расходов в формате [Метаданных](#/general#3-metadannye)                               |

Поле **noClosingDocs**

- Необязательное при создании, может быть передано в теле запроса.
- Если **noClosingDocs** не передано при создании, в ответе поле всегда присутствует со значением `false`.
- Если **noClosingDocs** установлено в `true`, к платежу нельзя привязать документы во взаиморасчетах (поле **operations** должно быть пустым).
  При попытке одновременно передать **noClosingDocs = true** и непустой **operations** возвращается ошибка **22003** с описанием:
  «Платеж не может одновременно иметь активный флаг “Без закрывающих документов” и привязанные документы».
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) Исходящих платежей.
...
| **attributes**                 | Массив объектов доп. полей Исходящих платежей в формате [Метаданных](#/general#3-metadannye) |
...
помечены `Только для чтения` в описании [атрибутов Исходящего платежа ](#/documents/payment-out#2-ishodyashij-platezh).
...
Поле **noClosingDocs** при изменении

- Может передаваться в теле запроса на изменение.
- Если при изменении других полей **noClosingDocs** не передано (в том числе для исторических документов), в ответе поле всегда присутствует и по умолчанию имеет значение `false`, если ранее не было установлено.
- Если в документе уже установлен **noClosingDocs = true** и в запросе передан непустой **operations**, сохранение завершится ошибкой **22003**.
- Если в запросе значение **noClosingDocs** меняется на `true` и одновременно передается непустой **operations**, сохранение завершится ошибкой **22003**.
- Если у документа уже есть связанные операции (**operations** не пустой) и в запросе значение **noClosingDocs** меняется на `true`, сохранение завершится ошибкой **22003**.
