...
| **store**               | [Meta](#/general#3-metadannye)  | `=` `!=`                                                                                                                                          | Метаданные склада. Если отгрузка проведена (applicable = true), склад становится обязательным для заполнения.<br> `+Обязательное при ответе` `+Expand`  `+Change-handler` `+Update-provider` |
| **sum**                 | Float                           | `=` `!=` `<` `>` `<=` `>=`                                                                                                                        | Сумма Отгрузки в копейках<br>`+Обязательное при ответе` `+Только для чтения`  `+Change-handler`                                                                                               |
...
| **declaration**        | Array(Object)                                             | Информация о прослеживаемости импортных товаров. [Подробнее тут](#/documents/demand#4-informaciya-o-dvizhenii-importnyh-tovarov)<br>`+Выводится по запросу` `+Только для чтения`                                                                                                                                                                       |
...
| **pack**               | Object                                                    | фасовка Товара. [Подробнее тут](#/dictionaries/product#5-fasovki-tovara)  `+Change-handler` `+Update-provider`                                                                                                                                                                                                                                       |
...
| **things**             | Array(String)                                             | Серийные номера. Значение данного атрибута игнорируется, если товар позиции не находится на серийном учете. В ином случае количество товаров в позиции будет равно количеству серийных номеров, переданных в значении атрибута. Если отгрузка проведена (applicable = true), серийные номера становятся обязательными для заполнения у товаров с серийным учетом. `+Change-handler` |
| **trackingCodes**      | Array(Object)                                             | Коды маркировки товаров и транспортных фасовок. [Подробнее тут](#/documents/demand#4-kody-markirovki-tovarov-i-transportnyh-upakovok)                                                                                                                                                                                                                 |
...
С позициями можно работать с помощью [специальных ресурсов для управления позициями Отгрузки](#/documents/demand#3-upravlenie-poziciyami-otgruzki),
...
#### Информация о движении импортных товаров
Поле **declaration** выводится по запросу. Для вывода в позициях документа информации о движении импортных товаров 
необходимо передать в URL запроса дополнительный параметр `fields=declaration`, например `../demand/{id}/positions?fields=declaration`. [Подробнее о параметре fields](#/general#3-chto-takoe-fields).

Аттрибуты объекта:

| Название     | Тип                            | Описание                                                                                |
|--------------|:-------------------------------|:----------------------------------------------------------------------------------------|
| **gtd**      | String                         | Грузовая таможенная декларация<br>`+Только для чтения`                                  |
| **rnpt**     | String                         | Регистрационный номер партии товара (прослеживаемый товар)<br>`+Только для чтения`      |
...
#### Коды маркировки товаров и транспортных фасовок в формате тега 1162
Поддержаны в виде иерархической структуры JSON. 
...
+ **assortment** - Ссылка на товар/услугу/партию/модификацию/комплект, которую представляет собой позиция.
  Подробнее об этом поле можно прочитать в описании [позиции Отгрузки](#/documents/demand#4-pozicii-otgruzki)
