...
| **store**               | [Meta](#/general#3-metadannye)  | `=` `!=`                                                                                                                                          | Метаданные склада. Если отгрузка проведена (applicable = true), склад становятся обязательными для заполнения.<br> `+Обязательное при ответе` `+Expand`  `+Change-handler` `+Update-provider` |
