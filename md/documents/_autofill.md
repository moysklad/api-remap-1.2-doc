...
Кодом для эндпоинта Автозаполнения в составе JSON API является ключевое слово `wizard`. С помощью эндпоинта можно рассчитать значение скидок, цен и ндс для позиций следующих документов:
...
- [Оприходование](#/documents/enter#2-oprihodovanie)
- [Списание](#/documents/loss#2-spisanie)
- [Начисления зарплаты](#/documents/payroll#2-nachislenie-zarplaty)
...
Позиции в шаблоне - это список товаров/услуг/модификаций/партий/комплектов.
...
+ **assortment** - Ссылка на товар/услугу/партию/модификацию/комплект, которую представляет собой позиция, в формате [Метаданных](#/general#3-metadannye)
...
Запрос автозаполнения с параметром `action` со значением `evaluate_price`. Требуется заполнение поля **agent** (или **organization**, если поле **agent** отсутствует). 
...
Запрос автозаполнения с параметром `action` со значением `evaluate_discount`. Требуется заполнение поля **agent** (или **organization**, если поле **agent** отсутствует). 
...
Запрос автозаполнения с параметром `action` со значением `evaluate_vat`. Требуется заполнение поля **organization**. 
...
Запрос автозаполнения с параметром `action` со значением `evaluate_cost`. Выполняется только для Возвратов Покупателя и Розничных Возвратов *без основания*. Требуется заполнение поля **store**. 
