...
Запрос автозаполнение с параметром `action` со значением `evaluate_price`. Требуется заполнение поля **agent** (или **organization**, если поле **agent** отсутствует). 
...
### Запрос автозаполнение скидок

Запрос автозаполнение с параметром `action` со значением `evaluate_discount`. Требуется заполнение поля **agent** (или **organization**, если поле **agent** отсутствует). 
...
### Запрос автозаполнение НДС

Запрос автозаполнение с параметром `action` со значением `evaluate_vat`. Требуется заполнение поля **organization**. 
...
### Запрос автозаполнение себестоимости

Запрос автозаполнение с параметром `action` со значением `evaluate_cost`. Выполняется только для Возвратов Покупателя и Розничных Возвратов *без основания*. Требуется заполнение поля **store**. 
