...
| **meta**                | [Meta](#/general#3-metadannye)                             |                                                                                                                                                   | Метаданные Входящего платежа<br>`+Обязательное при ответе`                                                                                                      |
...
| **state**               | [Meta](#/general#3-metadannye)                             | `=` `!=`                                                                                                                                          | Метаданные статуса Входящего платежа<br>`+Expand`                                                                                                               |
| **sum**                 | Float                                                      | `=` `!=` `<` `>` `<=` `>=`                                                                                                                        | Сумма Входящего платежа в установленной валюте<br>`+Обязательное при ответе`                                                                                    |
...
| Название                       | Описание                                                                                                                                |
| ------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **factureOut**                 | Ссылка на Счет-фактуру выданный, с которым связан этот платеж в формате [Метаданных](#/general#3-metadannye) |
| **operations**                 | Коллекция метаданных ( [Метаданные](#/general#3-metadannye)) + поле linkedSum на связанные операции                           |
...
О работе с доп. полями Входящих платежей можно прочитать [здесь](#/general#3-rabota-s-dopolnitelnymi-polyami)

#### Ограничение по валюте документа и счёта организации

Если в платеже заполнено поле **organizationAccount** и счёт организации в валюте, отличной от валюты учёта компании, то валюта документа (объект **rate**) должна совпадать с валютой этого счёта. Иначе при сохранении вернётся ошибка [22004](#/errors#3-kody-oshibok-dlya-platezhej). Для приходного и расходного ордера валидация по счёту организации не применяется.
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) Входящих платежей.
...
| **attributes**                 | Массив объектов доп. полей Входящих платежей в формате [Метаданных](#/general#3-metadannye) |
...
помечены `Только для чтения` в описании [атрибутов Входящего платежа](#/documents/payment-in#2-vhodyashij-platezh).
