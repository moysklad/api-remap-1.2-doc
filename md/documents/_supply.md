...
| **meta**                | [Meta](#/general#3-metadannye)                            |                                                                                                                                                   | Метаданные Приемки<br>`+Обязательное при ответе` `+Change-handler`                                                                                                                      |
...
| **overhead**            | Object                                                    |                                                                                                                                                   | Накладные расходы. [Подробнее тут](#/documents/supply#4-nakladnye-rashody). Если Позиции Приемки не заданы, то накладные расходы нельзя задать<br>`+Update-provider` |
...
| **state**               | [Meta](#/general#3-metadannye)                            | `=` `!=`                                                                                                                                          | Метаданные статуса Приемки<br>`+Expand` `+Change-handler` `+Update-provider`                                                                                                            |
...
| **sum**                 | Float                                                     | `=` `!=` `<` `>` `<=` `>=`                                                                                                                        | Сумма Приемки в копейках<br>`+Обязательное при ответе` `+Только для чтения` `+Change-handler`                                                                                           |
...
| Название            | Описание                                                                                                        |
|---------------------|:----------------------------------------------------------------------------------------------------------------|
| **purchaseOrder**   | Ссылка на связанный заказ поставщику в формате [Метаданных](#/general#3-metadannye)                             |
| **factureIn**       | Ссылка на Счет-фактуру полученный, с которым связана эта Приемка в формате [Метаданных](#/general#3-metadannye) |
...
| **productionTask**  | Ссылка на связанное производственное задание в формате [Метаданных](#/general#3-metadannye)                     |
| **returns**         | Массив ссылок на связанные возвраты в формате [Метаданных](#/general#3-metadannye)                              |

#### Позиции Приемки
Позиции Приемки - это список товаров/услуг/модификаций/партий.
...
| **country**       | [Meta](#/general#3-metadannye)                            | Метаданные страны<br>`+Expand`                                                                                                                                                                                                                           |
...
| **gtd**           | Object                                                    | ГТД. [Подробнее тут](#/dictionaries/gtd#2-gruzovaya-tamozhennaya-deklaraciya-gtd)                                                                                                                                                                                                                                                   |
...
| **trackingCodes** | Array(Object)                                             | Коды маркировки товаров и транспортных фасовок. [Подробнее тут](#/documents/supply#4-kody-markirovki-tovarov-i-transportnyh-upakovok)                                                                                                |
...
О работе с доп. полями Приемок можно прочитать [здесь](#/general#3-rabota-s-dopolnitelnymi-polyami)

#### Коды маркировки товаров и транспортных фасовок
...
Для каждого кода указывается тип **type**: `trackingcode` (код маркировки товара), `consumerpack` (код маркировки потребительской фасовки) или `transportpack` (код транспортной фасовки). 
Допустима вложенность кодов маркировки товаров в транспортные фасовки. Транспортные фасовки не могут иметь вложенных фасовок. 
Коды фасовок могут отсутствовать - в этом случае структура не будет вложенной. 
...
[Массовое создание и обновление](#/general#3-sozdanie-i-obnovlenie-neskolkih-obuektov) Приемок.
...
| **attributes**                 | Массив объектов доп. полей Приемок в формате [Метаданных](#/general#3-metadannye) |
...
помечены `Только для чтения` в описании [атрибутов Приемки](#/documents/supply#2-priemka).
...
```shell
curl --compressed -X GET \
  "https://api.moysklad.ru/api/remap/1.2/entity/supply/63918a49-886e-11ea-0a80-151b0000007f?expand=positions" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip"
```
...
+ **assortment** - Ссылка на товар/услугу/партию/модификацию, которую представляет собой позиция.
  Подробнее об этом поле можно прочитать в описании [позиции Приемки](#/documents/supply#4-pozicii-priemki)
...
```shell
curl --compressed -X POST \
  "https://api.moysklad.ru/api/remap/1.2/entity/supply/63918a49-886e-11ea-0a80-151b0000007f/positions" \
  -H "Authorization: Basic <Credentials>" \
  -H "Accept-Encoding: gzip" \
  -H "Content-Type: application/json" \
    -d '{
