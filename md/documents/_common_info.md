...
Например, для шаблона отгрузки - смотреть в [Шаблонах отгрузки](#/documents/demand#3-zaprosy-shablon-otgruzki)
...
Из корзины документ восстанавливается только средствами UI (возможность восстановления через API отсутствует).
...
К некоторым документам в JSON API могут быть привязаны платежи. Платежи бывают 4-х типов: [Входящий платеж](#/documents/payment-in#2-vhodyashij-platezh), [Приходный ордер](#/documents/cashin#2-prihodnyj-order), [Исходящий платеж](#/documents/payment-out#2-ishodyashij-platezh), [Расходный ордер](#/documents/cashout#2-rashodnyj-order).
...
В результате привязки платежа, в составе коллекции **operations** платежа появится новый объект, указывающий на документ, а в составе коллекции **payments** у документа, к которому
...
| ------------ | :-------------------------------------------------------- | :----------------------------------------------------------------------------------- |
| **currency** | [Meta](#/general#3-metadannye) | Метаданные валюты<br>`+Обязательное при ответе` `+Expand`                            |
