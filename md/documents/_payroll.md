...
| **description**        | String(4096)                   | Описание Начисления зарплаты. <br>                                                                                                                                                                                                                     |
| **attributes**         | Array(Object)                  | Коллекция метаданных доп. полей. [Поля объекта](#/general#3-rabota-s-dopolnitelnymi-polyami)<br>                                                                                                                                                       |
...
| **startPayrollPeriod** | DateTime                       | Начало расчётного периода, за который начисляется зарплата. *Разница начала и окончания расчётного периода должна быть кратна 24 часам и при этом игнорируются секунды*<br>`+Обязательное при ответе` `+Необходимо при создании` `+Update-provider`    |
| **endPayrollPeriod**   | DateTime                       | Окончание расчётного периода, за который начисляется зарплата. *Разница начала и окончания расчётного периода должна быть кратна 24 часам и при этом игнорируются секунды*<br>`+Обязательное при ответе` `+Необходимо при создании` `+Update-provider` |
...
| **startPayrollPeriod** | Начало расчётного периода, за который начисляется зарплата           |
| **endPayrollPeriod**   | Окончание расчётного периода, за который начисляется зарплата        |
